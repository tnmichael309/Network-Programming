...
void clientPools::closeOtherSocketFD(int curID){

	int curFD = findUserFD(curID);
	...
	for(int i = 0; i < m_vClients.size(); i++){
		if(m_vClients[i].isActiveUser() == true && m_vClients[i].getSockfd() != curFD){
			close(m_vClients[i].getSockfd());
			cout << m_vClients[i].getSockfd() << endl;
		}
	}
};
